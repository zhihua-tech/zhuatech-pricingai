 ...
 */
@SpringBootTest @AutoConfigureMockMvc class PricingAiApiIntegrationTests {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanReadShopfloorDashboard()throws Exception{mvc.perform(get("/api/shopfloor/dashboard").header("Authorization","Bearer "+operatorToken)).andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.metrics[0].label").value("定价对象"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanRecommendPrice()throws Exception{mvc.perform(post("/api/ai/pricing/recommend").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"skuCode\":\"SKU-001\",\"unitCost\":60,\"currentPrice\":120,\"competitorMedianPrice\":110,\"minimumMarginRate\":0.25,\"inventoryCoverDays\":120,\"priceElasticity\":1.8}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.action").value("LOWER")).andExpect(jsonPath("$.data.approvalRequired").value(true));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
